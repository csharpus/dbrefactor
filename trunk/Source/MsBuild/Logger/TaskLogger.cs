...
namespace DbRefactor.MSBuild.Logger
{
	/// <summary>
	...
	public class TaskLogger : ILogger
	{
		...
		{
			task.Log.LogMessage(Microsoft.Build.Framework.MessageImportance.Normal, format, args);
		}
		
		protected void LogError(string format, params object[] args)
		{
			task.Log.LogMessage(Microsoft.Build.Framework.MessageImportance.High, format, args);
		}
		...
		{
			task.Log.LogMessage( Microsoft.Build.Framework.MessageImportance.Normal,
				"{0} {1}", "".PadLeft(WidthFirstColumn), String.Format(format, args));
		}

		public void Modify(string query)
		{
			task.Log.LogMessageFromText(query, Microsoft.Build.Framework.MessageImportance.Normal);
		}

...
namespace DbRefactor.Runner
{
	...
	internal class DatabaseMigrationTarget : MigrationTarget
	{
		...
		public DatabaseMigrationTarget(TransformationProvider provider, IDatabase database,
		                               IDatabaseEnvironment databaseEnvironment, SchemaHelper schemaHelper, string category)
		{
		...
		{
			if (!schemaHelper.TableExists("SchemaInfo")) return 0;
		...
		{
			if (schemaHelper.TableExists("SchemaInfo")) return;

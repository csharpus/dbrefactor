...
{
	internal class MySqlConstraintQueryBuilder
	{
		private readonly ConstraintFilter filter;
		private readonly List<string> restrictions = new List<string>();

		public MySqlConstraintQueryBuilder(ConstraintFilter filter)
		{
			this.filter = filter;
		}
...
inner join INFORMATION_SCHEMA.TABLE_CONSTRAINTS as Constraints on Constraints.CONSTRAINT_NAME = `Keys`.CONSTRAINT_NAME
where NOT `Keys`.CONSTRAINT_NAME = 'PRIMARY' AND NOT `Keys`.CONSTRAINT_NAME = 'name'
				";
			AddTableRestriction();
